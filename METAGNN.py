...
class META_GCN(nn.Module):
    def __init__(self, userNum, itemNum, behavior, behavior_mats):
        ...
        for i in range(0, len(self.gnn_layer)):  
            self.layers.append(META_GCNLayer(args.hidden_dim, args.hidden_dim, self.userNum, self.itemNum, self.behavior, self.behavior_mats))  
...
# #     else:
# #         padding = torch.full((max_length - sequence.numel(),), padding_value)
...
#         # user_embed, item_embed, user_embeds, item_embeds = self.gru()
#         user_embed, item_embed, user_embeds, item_embeds = self.gcn()
...
#         self.userNum = userNum
#         self.itemNum = itemNum
#         self.embedding_size = args.hidden_dim 
...
#             # with open('/home/joo/CML/data/IJCAI_15/{}_ijcai_META_gru_padding.pkl'.format(i), 'wb') as file:    # james.p 파일을 바이너리 쓰기 모드(wb)로 열기
#             #     pickle.dump(result, file)
#             #     print(file)
# ##########################################################################################################################################################
            
#             result = pickle.load(open('/home/joo/CML/data/Tmall/{}_REAL_META_gru_padding.pkl'.format(i), 'rb')).to(device)
#             # print("gru_METAPATH")
...
#         # self.user_embedding_table =  torch.nn.Embedding(self.userNum, self.embedding_size).to('cuda:1')
#         # self.user_embedding_table =  torch.nn.Embedding(self.userNum, self.embedding_size).to(device)  # Embedding(31882, 16)
#         self.user_embedding_table =  torch.nn.Embedding(self.itemNum, self.embedding_size).to(device)
#         # print("self.user_embedding_table :" ,self.user_embedding_table )
#         # self.item_embedding_table =  torch.nn.Embedding(self.itemNum, self.embedding_size).to(device) # Embedding(31232, 16)
#         # self.gru_model = GRUModel(in_dim, self.embedding_size, out_dim) 
#         self.user_gru_model = GRUModel(self.userNum, self.embedding_size, self.userNum)  
...
#             # result = torch.cat(padded_item_indices, dim=0).long()
#             # # print("padded_item_indices_cat_result:",result.shape)  #torch.Size([31882, 293])
#             # with open('/home/joo/JOOCML/data/retail_rocket/{}_REAL_META_gru_padding.pkl'.format(i), 'wb') as file:    # james.p 파일을 바이너리 쓰기 모드(wb)로 열기
#             #     pickle.dump(result, file)
#             #     print(file)
# ##########################################################################################################################################################
#             #/home/joo/JOOCML/data/retail_rocket/0_Real_META_gru_padding.pkl
#             result = pickle.load(open('/home/joo/JOOCML/data/retail_rocket/{}_Real_META_gru_padding.pkl'.format(i), 'rb')).to(device)
#             # print("gru_METAPATH")
#             gru_user_embeddings= self.user_embedding_table(result).to(device) #shape:torch.Size([4, 31882, 16])   shape:torch.Size([31882, 63, 16])     
#             # print('Device:', device)   
#                 # print("gru_user_embeddings", gru_user_embeddings.shape)   
...
#             gru_u_embed_list[i] = self.user_gru_model(gru_user_embeddings).to(device)
#             # print("gru_u_embed_list[i] :", gru_u_embed_list[i].shape)  # gru_u_embed_list[i] :  torch.Size([31882, 16])
#             # print(gru_u_embed_list[i])
